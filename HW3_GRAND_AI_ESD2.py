...
# --- Error-support-distilled graph-aware ROI selector (offline-calibrated, online-lightweight) ---
class ErrorSupportDistillManager:
    """Collect supervised error-support labels and fit a tiny logistic scorer.
    ...
    def observe_support(self,
                        feature_matrix: Optional[np.ndarray],
                        feature_vars: Optional[np.ndarray],
                        positive_vars: Optional[np.ndarray]) -> None:
        if not self.collect:
            return
        if feature_matrix is None or feature_vars is None or positive_vars is None:
            return
        X = np.asarray(feature_matrix, dtype=np.float64)
        vars_arr = np.asarray(feature_vars, dtype=np.int32).reshape(-1)
        pos_vars = np.asarray(positive_vars, dtype=np.int32).reshape(-1)
        if X.ndim != 2 or X.shape[0] != vars_arr.size or X.shape[0] == 0:
            return
        if pos_vars.size == 0:
            return
        y = np.isin(vars_arr, pos_vars).astype(np.float64)
        pos_idx = np.flatnonzero(y > 0.5)
        if pos_idx.size == 0:
            return
        neg_idx = np.flatnonzero(y <= 0.5)
        max_neg = int(max(16, round(float(self.neg_ratio) * float(pos_idx.size))))
    ...
    def observe_success(self, feature_matrix: Optional[np.ndarray], feature_vars: Optional[np.ndarray], flipped_vars: Optional[np.ndarray]) -> None:
        ...
        self.observe_support(feature_matrix, feature_vars, flipped_vars)
    def finalize(self) -> None:
        ...
        if self.success_examples < self.min_successes:
            print(f"[TDISTILL] WARNING: only {self.success_examples} support-labeled calibration frames; keeping heuristic fallback.")
            return
...
_TEACHER_DISTILL_MANAGER: Optional[ErrorSupportDistillManager] = None


def _get_teacher_distill_manager() -> ErrorSupportDistillManager:
    global _TEACHER_DISTILL_MANAGER
    if _TEACHER_DISTILL_MANAGER is None:
        _TEACHER_DISTILL_MANAGER = ErrorSupportDistillManager()
...
def _select_search_vars_ai_teacher_distill_roi(union_vars: np.ndarray,
    ...
    """Error-support-distilled graph-aware selector.

    Cold start: exactly fall back to the empirically strongest current regime
    (``ai_mix_roi``) while collecting supervised error-support labels.
    ...
    if L <= 0 or union_vars.size == 0:
        return np.array([], dtype=np.int32), {
            "selection_mode_used": "ai_error_support_distill_roi_v2",
            "sv_seeded_count": 0,
            "sv_neighbor_visits": 0,
            "sv_score_len": int(union_vars.size),
            "td_model_ready": 0,
    ...
    if not tdm.model_ready():
        ...
        meta = dict(base_meta)
        meta["selection_mode_used"] = "ai_error_support_distill_roi_v2"
    ...
    fused = 0.88 * probs + 0.12 * (base_score / float(max(1.0e-9, float(base_score.max()) if base_score.size else 1.0)))
    ...
    top_blocks = 2 if compact else (3 if diffuse else 2)
...
def _select_search_vars_ai_tanner_trap_roi(union_vars: np.ndarray,
                                           unsat_checks: np.ndarray,
                                           code_cfg: CodeConfig,
                                           llr_for_sort: np.ndarray,
                                           L: int,
                                           ...
                                           llr_snapshot: Optional[np.ndarray] = None,
                                           llr_channel: Optional[np.ndarray] = None) -> Tuple[np.ndarray, Dict[str, int]]:
    """Trap-set oriented Tanner selector: connected graph core + tiny scout list.

    Motivation from the live TG2 results:
      - remaining stage-1 failures are diffuse in codeword index, so block-local
        concentration alone is a weak signal;
      - however, these failures can still be compact on the Tanner graph as
        small communities of unsatisfied checks and variable nodes.

    Runtime path:
      1) use the Tanner-ROI ordering as a cheap teacher prior;
      2) score unsatisfied checks and variables by shared-check overlap,
         teacher rank, cycle/community pressure, and channel/posterior
         disagreement;
      3) keep a connected trapping-set core plus a tiny block-diverse scout set.
    """
    union_vars = np.asarray(union_vars, dtype=np.int32)
    unsat_checks = np.asarray(unsat_checks, dtype=np.int32)
    L = int(max(L, 0))
    if L <= 0 or union_vars.size == 0:
        return np.array([], dtype=np.int32), {
            "selection_mode_used": "ai_tanner_trap_roi",
            "sv_seeded_count": 0,
            "sv_neighbor_visits": 0,
            "sv_score_len": int(union_vars.size),
            "ai_tg3_profile": "empty",
            "ai_tg3_prefilter": 0,
            "ai_tg3_local_budget": 0,
            "ai_tg3_global_budget": 0,
            "ai_tg3_seed_vars": 0,
            "ai_tg3_seed_checks": 0,
            "ai_tg3_local_size": 0,
            "ai_tg3_blocks_used": 0,
        }

    pre_scale = float(getattr(cfg, "ai_tg3_prefilter_scale", 3.2) or 3.2)
    pre_extra = max(8, int(getattr(cfg, "ai_tg3_prefilter_extra", max(16, L)) or max(16, L)))
    pre_cap = int(min(union_vars.size, max(L, int(round(pre_scale * max(L, 1))), int(L + pre_extra))))
    ...
    if ordered_vars.size <= L:
        ...
        meta.update({
            "selection_mode_used": "ai_tanner_trap_roi",
            "ai_tg3_profile": str(meta0.get("ai_tanner_profile", "teacher_only")),
            "ai_tg3_prefilter": int(ordered_vars.size),
            "ai_tg3_local_budget": int(min(L, ordered_vars.size)),
            "ai_tg3_global_budget": 0,
            "ai_tg3_seed_vars": int(min(ordered_vars.size, L)),
            "ai_tg3_seed_checks": 0,
            "ai_tg3_local_size": int(ordered_vars.size),
            "ai_tg3_blocks_used": 0,
        })
        return ordered_vars[:L], meta

    block_size = max(1, int(getattr(cfg, "ai_tg3_block_size", getattr(cfg, "ai_tg2_block_size", 64)) or 64))
    union_size = int(union_vars.size)
    block_conc = _ai_rank_roi_block_concentration(union_vars, block_size)
    diffuse = (
        float(union_size) >= float(int(getattr(cfg, "ai_tg3_diffuse_union_size", 240) or 240))
        and float(block_conc) <= float(getattr(cfg, "ai_tg3_diffuse_block_concentration", 0.07) or 0.07)
    )
    compact = float(block_conc) >= float(getattr(cfg, "ai_tg3_compact_block_concentration", 0.10) or 0.10)

    if compact:
        local_share = float(getattr(cfg, "ai_tg3_local_share_compact", 0.90) or 0.90)
        seed_vars_k = int(getattr(cfg, "ai_tg3_seed_vars_compact", 8) or 8)
        top_checks_k = int(getattr(cfg, "ai_tg3_top_checks_compact", 6) or 6)
        profile = "compact_trap"
    elif diffuse:
        local_share = float(getattr(cfg, "ai_tg3_local_share_diffuse", 0.78) or 0.78)
        seed_vars_k = int(getattr(cfg, "ai_tg3_seed_vars_diffuse", 16) or 16)
        top_checks_k = int(getattr(cfg, "ai_tg3_top_checks_diffuse", 12) or 12)
        profile = "diffuse_trap"
    else:
        local_share = float(getattr(cfg, "ai_tg3_local_share_balanced", 0.84) or 0.84)
        seed_vars_k = int(getattr(cfg, "ai_tg3_seed_vars_balanced", 12) or 12)
        top_checks_k = int(getattr(cfg, "ai_tg3_top_checks_balanced", 8) or 8)
        profile = "balanced_trap"

    ordered_list = [int(v) for v in ordered_vars.tolist()]
    order_rank = {int(v): i for i, v in enumerate(ordered_list)}
    unsat_list = [int(c) for c in unsat_checks.tolist()]
    unsat_set = set(unsat_list)

    llr_snap = np.asarray(llr_snapshot if llr_snapshot is not None else llr_for_sort, dtype=np.float32)
    llr_chan = np.asarray(llr_channel, dtype=np.float32) if llr_channel is not None else None

    teacher_w = float(getattr(cfg, "ai_tg3_teacher_weight", 0.55) or 0.55)
    overlap_w = float(getattr(cfg, "ai_tg3_overlap_weight", 1.00) or 1.00)
    cycle_w = float(getattr(cfg, "ai_tg3_cycle_weight", 0.35) or 0.35)
    disagreement_w = float(getattr(cfg, "ai_tg3_disagreement_weight", 0.30) or 0.30)
    conflict_bonus = float(getattr(cfg, "ai_tg3_conflict_bonus", 0.35) or 0.35)

    check_scores: List[Tuple[float, int]] = []
    check_score_map: Dict[int, float] = {}
    check_member_cache: Dict[int, List[int]] = {}
    for c in unsat_list:
        members = [int(v) for v in code_cfg.checks_to_vars[int(c)] if int(v) in order_rank]
        if not members:
            continue
        check_member_cache[int(c)] = members
        ranks = sorted(order_rank[int(v)] for v in members)
        top_ranks = ranks[:min(6, len(ranks))]
        teacher_mass = float(sum(float(pre_cap - r) for r in top_ranks)) / float(max(1, len(top_ranks)))
        overlap_bonus = 0.18 * float(max(0, min(len(members), 5) - 1))
        weak_bonus = 0.0
        for v in members[:min(8, len(members))]:
            weak_bonus += 1.0 / max(0.5, abs(float(llr_for_sort[int(v)])))
        weak_bonus /= float(max(1, min(8, len(members))))
        score = teacher_mass + overlap_bonus + 0.15 * weak_bonus
        check_scores.append((float(score), int(c)))
        check_score_map[int(c)] = float(score)
    check_scores.sort(key=lambda t: (-float(t[0]), int(t[1])))

    chosen_checks = {int(c) for _score, c in check_scores[:max(1, top_checks_k)]}

    var_scores: Dict[int, float] = {}
    overlap_hits: Dict[int, int] = {}
    for v in ordered_list:
        neigh = [int(c) for c in code_cfg.vars_to_checks[int(v)] if int(c) in unsat_set]
        if not neigh:
            continue
        teacher_score = float(pre_cap - order_rank[int(v)]) / float(max(1, pre_cap))
        chosen_overlap = [c for c in neigh if c in chosen_checks]
        overlap_mass = float(sum(check_score_map.get(int(c), 0.0) for c in chosen_overlap))
        overlap_hits[int(v)] = int(len(chosen_overlap))
        cycle_pressure = 0.0
        for c in neigh:
            members = check_member_cache.get(int(c))
            if members is None:
                members = [int(u) for u in code_cfg.checks_to_vars[int(c)] if int(u) in order_rank]
                check_member_cache[int(c)] = members
            cycle_pressure += float(max(0, min(len(members), 5) - 1))
        disagreement = 0.0
        if llr_chan is not None:
            ps = float(llr_snap[int(v)])
            pc = float(llr_chan[int(v)])
            if (ps != 0.0) and (pc != 0.0) and (ps * pc < 0.0):
                disagreement = 1.0
        weak = 1.0 / max(0.5, abs(float(llr_for_sort[int(v)])))
        score = (
            teacher_w * teacher_score
            + overlap_w * overlap_mass
            + cycle_w * cycle_pressure
            + disagreement_w * disagreement
            + conflict_bonus * weak
        )
        var_scores[int(v)] = float(score)

    if not var_scores:
        meta = dict(meta0)
        meta.update({
            "selection_mode_used": "ai_tanner_trap_roi",
            "ai_tg3_profile": "fallback_teacher_only",
            "ai_tg3_prefilter": int(pre_cap),
            "ai_tg3_local_budget": int(min(L, ordered_vars.size)),
            "ai_tg3_global_budget": 0,
            "ai_tg3_seed_vars": 0,
            "ai_tg3_seed_checks": int(len(chosen_checks)),
            "ai_tg3_local_size": int(min(L, ordered_vars.size)),
            "ai_tg3_blocks_used": 0,
        })
        return ordered_vars[:L], meta
    ...
    seed_pool = sorted(
        var_scores.keys(),
        key=lambda v: (-float(var_scores.get(int(v), -1e18)), -int(overlap_hits.get(int(v), 0)), int(order_rank.get(int(v), pre_cap)), int(v))
    )
    seed_vars = [int(v) for v in seed_pool[:min(seed_vars_k, len(seed_pool))]]

    seed_touch_count: Dict[int, int] = {}
    for v in seed_vars:
        for c in code_cfg.vars_to_checks[int(v)]:
            ci = int(c)
            if ci in unsat_set:
                seed_touch_count[ci] = int(seed_touch_count.get(ci, 0) + 1)
                chosen_checks.add(ci)

    ranked_checks = sorted(
        chosen_checks,
        key=lambda c: (
            -float(check_score_map.get(int(c), 0.0)),
            -int(seed_touch_count.get(int(c), 0)),
            int(c),
        ),
    )
    keep_checks = set()
    for c in ranked_checks:
        if len(keep_checks) >= max(1, 2 * top_checks_k):
            break
        if seed_touch_count.get(int(c), 0) >= 2 or len(keep_checks) < max(1, top_checks_k):
            keep_checks.add(int(c))
    chosen_checks = keep_checks if keep_checks else set(ranked_checks[:max(1, top_checks_k)])

    radius = max(1, int(getattr(cfg, "ai_tg3_radius", 2) or 2))
    local_vars = set(int(v) for v in seed_vars)
    frontier_vars = set(int(v) for v in seed_vars)
    frontier_checks = set(int(c) for c in chosen_checks)
    ...
    for _hop in range(radius):
        ...
        for c in list(frontier_checks):
            ...
            members.sort(
                key=lambda v: (
                    -float(var_scores.get(int(v), -1e18)),
                    -int(overlap_hits.get(int(v), 0)),
                    int(order_rank.get(int(v), pre_cap)),
                    int(v),
                )
            )
            keep_n = max(4, min(len(members), max(8, int(getattr(cfg, "ai_tg3_min_local_budget", 12) or 12))))
            for v in members[:keep_n]:
                local_vars.add(int(v))
                next_vars.add(int(v))
        next_checks = set()
        for v in list(frontier_vars | next_vars):
            neigh = [int(c) for c in code_cfg.vars_to_checks[int(v)] if int(c) in unsat_set]
            neigh.sort(key=lambda c: (-float(check_score_map.get(int(c), 0.0)), -int(seed_touch_count.get(int(c), 0)), int(c)))
            for ci in neigh[:max(1, top_checks_k)]:
                next_checks.add(int(ci))
        frontier_vars = next_vars
        frontier_checks = next_checks - chosen_checks
        chosen_checks |= next_checks
        if not frontier_vars and not frontier_checks:
            break

    neighbor_blocks = max(0, int(getattr(cfg, "ai_tg3_neighbor_blocks", 1) or 1))
    chosen_blocks = set()
    for v in seed_vars:
        blk = int(v) // block_size
        for b in range(int(blk) - neighbor_blocks, int(blk) + neighbor_blocks + 1):
    ...
    local_order = sorted(
        [int(v) for v in ordered_list if int(v) in local_vars],
        key=lambda v: (
            -float(var_scores.get(int(v), -1e18)),
            -int(overlap_hits.get(int(v), 0)),
            int(order_rank.get(int(v), pre_cap)),
    ...
    min_local = max(6, int(getattr(cfg, "ai_tg3_min_local_budget", 12) or 12))
    local_budget = int(min(L, max(min_local, round(float(L) * np.clip(local_share, 0.40, 0.95)))))
    local_budget = min(local_budget, len(local_order))
    global_budget = max(0, int(L - local_budget))
    scout_extra = max(0, int(getattr(cfg, "ai_tg3_scout_global_extra", 8) or 8))
    ...
    used_blocks = {int(v) // block_size for v in selected}
    scout_diverse: List[int] = []
    scout_need = max(global_budget, min(scout_extra, max(0, L - len(selected))))
    for v in global_order:
        blk = int(v) // block_size
        if blk not in used_blocks:
            scout_diverse.append(int(v))
            used_blocks.add(int(blk))
        if len(scout_diverse) >= scout_need:
            break
    if len(scout_diverse) < scout_need:
        for v in global_order:
            vi = int(v)
    ...
    meta.update({
        "selection_mode_used": "ai_tanner_trap_roi",
        "ai_tg3_profile": str(profile),
        "ai_tg3_prefilter": int(pre_cap),
        "ai_tg3_local_budget": int(local_budget),
        "ai_tg3_global_budget": int(max(0, len(selected[:L]) - local_budget)),
        "ai_tg3_seed_vars": int(len(seed_vars)),
        "ai_tg3_seed_checks": int(len(chosen_checks)),
        "ai_tg3_local_size": int(len(local_order)),
        "ai_tg3_blocks_used": int(len(chosen_blocks)),
    })
    return np.asarray(selected[:L], dtype=np.int32), meta
...
def _select_presolver_vars(union_vars: np.ndarray,
    ...
    if selection_mode in ("ai_error_support_distill_roi_v2", "aiesd2", "error_support_distill_roi_v1", "receiver9_esd2", "aiteach3", "teacher_distill_roi_v3", "receiver9_teacher_v3",
                          "ai_teacher_distill_roi_v2", "aiteach2", "teacher_distill_roi_v2", "receiver9_teacher_v2",
                          "ai_teacher_distill_roi", "aiteach", "teacher_distill_roi", "receiver9_teacher"):
        base_vars, meta = _select_search_vars_ai_teacher_distill_roi(
            union_vars=union_vars,
            unsat_checks=unsat_checks,
            code_cfg=code_cfg,
            llr_for_sort=llr_for_sort,
...
def _apply_air_tg3_knobs(cfg: ClusterGrandConfig) -> None:
    cfg.ai_tg3_block_size = _get_int_env("GRAND_AIR_AI_TG3_BLOCK_SIZE", _get_int_env("GRAND_AIR_AI_TG2_BLOCK_SIZE", 64))
    cfg.ai_tg3_prefilter_scale = _get_float_env("GRAND_AIR_AI_TG3_PREFILTER_SCALE", 3.2)
    cfg.ai_tg3_prefilter_extra = _get_int_env("GRAND_AIR_AI_TG3_PREFILTER_EXTRA", 48)
    cfg.ai_tg3_diffuse_union_size = _get_int_env("GRAND_AIR_AI_TG3_DIFFUSE_UNION", 240)
    cfg.ai_tg3_diffuse_block_concentration = _get_float_env("GRAND_AIR_AI_TG3_DIFFUSE_BLOCK", 0.07)
    cfg.ai_tg3_compact_block_concentration = _get_float_env("GRAND_AIR_AI_TG3_COMPACT_BLOCK", 0.10)
    cfg.ai_tg3_local_share_diffuse = _get_float_env("GRAND_AIR_AI_TG3_LOCAL_SHARE_DIFFUSE", 0.78)
    cfg.ai_tg3_local_share_balanced = _get_float_env("GRAND_AIR_AI_TG3_LOCAL_SHARE_BALANCED", 0.84)
    cfg.ai_tg3_local_share_compact = _get_float_env("GRAND_AIR_AI_TG3_LOCAL_SHARE_COMPACT", 0.90)
    cfg.ai_tg3_seed_vars_diffuse = _get_int_env("GRAND_AIR_AI_TG3_SEED_VARS_DIFFUSE", 16)
    cfg.ai_tg3_seed_vars_balanced = _get_int_env("GRAND_AIR_AI_TG3_SEED_VARS_BALANCED", 12)
    cfg.ai_tg3_seed_vars_compact = _get_int_env("GRAND_AIR_AI_TG3_SEED_VARS_COMPACT", 8)
    cfg.ai_tg3_top_checks_diffuse = _get_int_env("GRAND_AIR_AI_TG3_TOP_CHECKS_DIFFUSE", 12)
    cfg.ai_tg3_top_checks_balanced = _get_int_env("GRAND_AIR_AI_TG3_TOP_CHECKS_BALANCED", 8)
    cfg.ai_tg3_top_checks_compact = _get_int_env("GRAND_AIR_AI_TG3_TOP_CHECKS_COMPACT", 6)
    cfg.ai_tg3_neighbor_blocks = _get_int_env("GRAND_AIR_AI_TG3_NEIGHBOR_BLOCKS", 1)
    cfg.ai_tg3_radius = _get_int_env("GRAND_AIR_AI_TG3_RADIUS", 2)
    cfg.ai_tg3_min_local_budget = _get_int_env("GRAND_AIR_AI_TG3_MIN_LOCAL_BUDGET", 12)
    cfg.ai_tg3_scout_global_extra = _get_int_env("GRAND_AIR_AI_TG3_SCOUT_GLOBAL_EXTRA", 8)
    cfg.ai_tg3_teacher_weight = _get_float_env("GRAND_AIR_AI_TG3_TEACHER_WEIGHT", 0.55)
    cfg.ai_tg3_overlap_weight = _get_float_env("GRAND_AIR_AI_TG3_OVERLAP_WEIGHT", 1.00)
    cfg.ai_tg3_cycle_weight = _get_float_env("GRAND_AIR_AI_TG3_CYCLE_WEIGHT", 0.35)
    cfg.ai_tg3_disagreement_weight = _get_float_env("GRAND_AIR_AI_TG3_DISAGREEMENT_WEIGHT", 0.30)
    cfg.ai_tg3_conflict_bonus = _get_float_env("GRAND_AIR_AI_TG3_CONFLICT_BONUS", 0.35)
...
_apply_air_tg3_knobs(grand_cfg_awgn_air_tiny)
...
_apply_air_tg3_knobs(grand_cfg_awgn_air_full)
...
_apply_air_tg3_knobs(grand_cfg_awgn_air_full_boost)
...
_apply_air_tg3_knobs(grand_cfg_awgn_air_fallback)
...
_apply_air_tg3_knobs(grand_cfg_awgn_air_fallback_boost)
...
def _error_support_distill_observe_from_attempt(frame: FrameLog, snapshot_iter: int, res: Optional[ClusterGrandResult]) -> None:
    if res is None:
        return
    ...
    if X is None or vars_arr is None:
        return
    snaps = getattr(frame, "snapshots", None)
    if not isinstance(snaps, dict):
        return
    hard_map = snaps.get("hard_bits", {})
    if not isinstance(hard_map, dict):
        return
    snap_i = int(getattr(res, "snapshot_iter_used", snapshot_iter) or snapshot_iter)
    hard_bits = hard_map.get(snap_i, None)
    if hard_bits is None:
        return
    true_bits = getattr(frame, "c_bits", None)
    if true_bits is None:
        return
    vars_np = np.asarray(vars_arr, dtype=np.int32).reshape(-1)
    if vars_np.size == 0:
        return
    hard_bits = np.asarray(hard_bits, dtype=np.uint8)
    true_bits = np.asarray(true_bits, dtype=np.uint8)
    positive_vars = vars_np[hard_bits[vars_np] != true_bits[vars_np]]
    if positive_vars.size == 0:
        return
    mgr.observe_support(X, vars_np, positive_vars)
                ...
                if policy_mode in ("probe_moe_roi_fix", "probe_moe_roi", "probe_moe", "probe_fix", "probe"):
                    ...
                    for snap_pos, snap in enumerate(gate_schedule, start=1):
                        ...
                        if res_probe is not None:
                            # ESD2: collect supervised error-support labels from *every*
                            # probe attempt, not only the rare probe successes. The true
                            # error support is available in simulation, so calibration
                            # should learn from all failed stage-1 frames in the useful
                            # early snapshot regime.
                            _error_support_distill_observe_from_attempt(frame, snap_i, res_probe)
                            res_final = res_probe
        ...
        if run_receiver9:
            for it in stage1_list:
                ...
                if policy_mode in ("distilled_tree_bandit", "tree_bandit", "dt_bandit"):
                    ...
                    if selection_mode in ("ai_error_support_distill_roi_v2", "aiesd2", "error_support_distill_roi_v1", "receiver9_esd2"):
                        dec_name = f"hybairesd2{int(it)}"
                    elif selection_mode in ("aiteach3", "teacher_distill_roi_v3", "receiver9_teacher_v3"):
                        dec_name = f"hybairtd3{int(it)}"
                    elif selection_mode in ("ai_teacher_distill_roi_v2", "aiteach2", "teacher_distill_roi_v2", "receiver9_teacher_v2"):
