...
class ErrorSupportDistillManager:
    ...
    def finalize(self) -> None:
        ...
        if self.success_examples < self.min_successes:
            print(f"[TDISTILL] WARNING: only {self.success_examples} support-labeled frames; keeping heuristic fallback.")
            return
...
def _select_search_vars_ai_teacher_distill_roi(union_vars: np.ndarray,
    ...
    if L <= 0 or union_vars.size == 0:
        return np.array([], dtype=np.int32), {
            "selection_mode_used": "ai_error_support_distill_roi_v1",
            "sv_seeded_count": 0,
            "sv_neighbor_visits": 0,
            "sv_score_len": int(union_vars.size),
            "td_model_ready": 0,
    ...
    if not tdm.model_ready():
        ...
        meta = dict(base_meta)
        meta["selection_mode_used"] = "ai_error_support_distill_roi_v1"
...
def _select_presolver_vars(union_vars: np.ndarray,
    ...
    if selection_mode in ("ai_error_support_distill_roi_v1", "aiesd1", "error_support_distill_roi_v1", "receiver9_esd1", "aiteach3", "teacher_distill_roi_v3", "receiver9_teacher_v3",
        ...
        if run_receiver9:
            for it in stage1_list:
                ...
                if policy_mode in ("distilled_tree_bandit", "tree_bandit", "dt_bandit"):
                    ...
                    if selection_mode in ("ai_error_support_distill_roi_v1", "aiesd1", "error_support_distill_roi_v1", "receiver9_esd1"):
                        dec_name = f"hybairesd{int(it)}"
